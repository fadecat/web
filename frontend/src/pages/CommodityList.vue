...
/* 表头吸顶(对齐 StockDividend 方案): 布局滚动容器是 AppLayout 的 .content,
   EP 默认 .el-table overflow:hidden 会自成滚动容器导致吸顶失效, 改 clip;
   外层不能包 overflow-x:auto(同样自成滚动容器), 窄屏横向滚动交给 EP 表体内部处理。
   漏风已由 AppLayout .content 用 margin-top(而非 padding-top) 全局修复 */
.commodity-table-wrap { padding: 0; }.commodity-table-wrap :deep(.el-table) { overflow: clip; font-size: 13px; font-variant-numeric: tabular-nums; --el-table-header-bg-color: var(--el-fill-color); }.commodity-table-wrap :deep(.el-table__header-wrapper) { position: sticky; top: 0; z-index: 3; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.08); }.commodity-table-wrap :deep(.el-table th), .commodity-table-wrap :deep(.el-table td) { height: 36px; padding: 0; white-space: nowrap; }.commodity-table-wrap :deep(.el-table__row) { cursor: pointer; }.commodity-table-wrap :deep(.section-row) { background: var(--el-fill-color-light); cursor: default; }.section-label { font-weight: 700; font-size: 13px; }.section-label small { margin-left: 8px; color: var(--el-text-color-secondary); font-weight: 400; font-size: 11px; }.commodity-table-wrap :deep(.instrument-column .cell) { text-align: left; }.commodity-table-wrap :deep(.instrument-column strong) { display: block; font-weight: 600; }.commodity-table-wrap :deep(.instrument-column small) { display: block; color: var(--el-text-color-secondary); font-size: 11px; }
