...
const columns=[
 {field:'code',label:'代码',width:68,align:'center',fixed:'left',sortable:false},
 {field:'name',label:'名称',width:128,fixed:'left',sortable:false},
 {field:'rank',label:'排名',width:58,align:'center',fixed:'left',sortable:true},
 {field:'stock_nm',label:'正股',width:106,align:'left',fixed:'left',sortable:false},
 {field:'industry_name',label:'行业',width:92,sortable:false},
 {field:'rating',label:'评级',width:62,align:'center',sortable:false},
 {field:'sprice',label:'正股价',width:70,align:'right',sortable:true},
 ...
 {field:'redeem_price',label:'赎回价',width:74,align:'right',sortable:true},
 {field:'total_score',label:'得分',width:62,align:'right',sortable:true},
];
...
function persist(){localStorage.setItem(storageKey,JSON.stringify({density:density.value}));}
watch(density,persist);
// 固定列响应式: 手机(≤700px, 与下方媒体查询同阈值)只固定名称列。
// 4左+1右固定列合计 448px, 在 ~370px 的手机可视区会把表体完全盖住,
// 滑动时中段列从固定列底下滑过、屏幕零响应(e84d7f6 引入, 此处回修)。
const isMobile=ref(false);
let _mql=null;
const _syncMobile=e=>{isMobile.value=e.matches;};
...
function display(row,field){if(field==='stock_financial_eps_growth_ttm'){const v=row.stock_financial?.eps_growth_ttm;return v==null?'—':`${Number(v).toFixed(2)}%`;}const v=row[field];if(v==null||v==='')return field==='rating'?'无评级':'—';return numberFields.has(field)?`${Number(v).toFixed(2)}${['simple_maturity_yield_pct','premium_rt','stock_financial_eps_growth_ttm'].includes(field)?'%':''}`:v;}
function sortValue(row,field){return field==='stock_financial_eps_growth_ttm'?row.stock_financial?.eps_growth_ttm:row[field];}
...
defineExpose({sortValue,sortBy,rows});
