...
const columns=[
 ...
 {field:'stock_financial_eps_growth_ttm',label:'净利润增长',width:94,default:true,align:'right',sortable:true},
...
const fieldAliases={stock_financial_profit:'stock_financial_eps_growth_ttm'};
const savedFields=Array.isArray(saved.columns)?saved.columns.map(f=>fieldAliases[f]||f).filter(f=>columns.some(c=>c.field===f)):[];
...
const visibleColumns=computed(()=>columns.filter(c=>selectedFields.value.includes(c.field)));
...
<template>
 <section class="results">
  ...
  <div class="tools"><el-radio-group v-model="view"><el-radio-button value="all">全部符合</el-radio-button><el-radio-button value="excluded">排除明细</el-radio-button></el-radio-group><el-input v-model="search" placeholder="当前结果内查找代码/名称" clearable/><el-button @click="sort={prop:'rank',order:'ascending'}">恢复默认排序</el-button><el-button @click="density=density==='compact'?'comfortable':'compact'">{{ density==='compact'?'舒适密度':'紧凑密度' }}</el-button><el-dropdown trigger="click" :hide-on-click="false" :teleported="false"><el-button>列设置</el-button><template #dropdown><el-dropdown-menu><div class="column-settings"><div class="column-group-title">默认列</div><label v-for="c in columns.filter(x=>x.default)" :key="c.field"><el-checkbox :model-value="selectedFields.includes(c.field)" :disabled="requiredFields.includes(c.field)" @change="toggleColumn(c.field)">{{ c.label }}</el-checkbox></label><div class="column-group-title optional-title">可选列</div><label v-for="c in columns.filter(x=>x.optional)" :key="c.field"><el-checkbox :model-value="selectedFields.includes(c.field)" @change="toggleColumn(c.field)">{{ c.label }}</el-checkbox></label><el-button link type="primary" @click="resetColumns">恢复默认</el-button></div></el-dropdown-menu></template></el-dropdown></div>
