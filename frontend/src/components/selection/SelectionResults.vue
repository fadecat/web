...
const columns=[{field:'rank',label:'排名',width:62},{field:'code',label:'代码',width:64},{field:'name',label:'名称',width:112},{field:'industry_name',label:'行业',width:96},{field:'rating',label:'评级',width:62},{field:'stock_nm',label:'正股名称',width:104},{field:'stock_financial_profit',label:'利润指标（集思录）',width:118},{field:'sprice',label:'正股',width:62},{field:'price',label:'价格',width:62},{field:'premium_rt',label:'溢价',width:62},{field:'simple_maturity_yield_pct',label:'收益率',width:88},{field:'year_left',label:'年限',width:62},{field:'curr_iss_amt',label:'规模(亿)',width:86},{field:'convert_price',label:'转股价',width:80},{field:'convert_value',label:'转股价值',width:86},{field:'dblow',label:'双低',width:62},{field:'redeem_price',label:'赎回价',width:74},{field:'pb',label:'市净率',width:74},{field:'redeem',label:'强赎',width:88},{field:'total_score',label:'得分',width:62}];
...
const numberFields=new Set(['price','sprice','redeem_price','simple_maturity_yield_pct','dblow','premium_rt','curr_iss_amt','convert_price','convert_value','year_left','pb','total_score']);
...
<template>
 <section class="results">
  ...
  <el-table :data="paged" stripe highlight-current-row max-height="620" @sort-change="sort=$event.prop&&$event.order?$event:{prop:'rank',order:'ascending'}">
