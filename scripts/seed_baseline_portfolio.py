...
def _register_and_sync(items, *, do_fetch: bool) -> list[int]:
    ...
    with SessionLocal() as db:
        for symbol, security_type, name in items:
            try:
                # `--symbols` 自定义时没有名称 → 传空, 由 register 按类型取真名
                # (场外基金走蛋卷详情, 股票/ETF 走腾讯行情); 取不到才回落代码, 不让 UI 显示空白
                row = portfolio_assets.register(symbol, security_type, name or "", db=db)
            except Exception as exc:  # noqa: BLE001 单标的注册失败不中断其余
                logger.error("注册失败 %s: %s", symbol, exc)
                continue
            ids.append(row["id"])
            print(f"  注册 {symbol}  {row['name'] or symbol}  ({'已存在' if not row['created'] else '新建'})")
