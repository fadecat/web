...
def save_dividend_yield_history(
    db: Session,
    index_code: str,
    ...
    参数:
        db: SQLAlchemy 会话
        index_code: 指数代码(config code, 与最新值落库口径一致)
        history: fetch_index_dividend_yield 返回的 history 字段
                 [{"date": "2017-05-26", "yield": 2.7286}, ...] 升序
    ...
    for item in history:
        ...
        db.add(IndexDividendYield(
            index_code=index_code,
            trade_date=trade_date,
            dividend_yield=item["yield"],
            # 历史行不带分位/均值(源 JSON 只有当日值序列), 分位列留空;
            # 前端展示用最新一行的分位即可
        ))
        inserted += 1
    db.commit()
