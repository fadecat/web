...
| 阶段 | 交付 | 依赖新数据源 | 依赖新表 |
| --- | --- | --- | --- |
| **P0** 标的注册 + 按需抓取（股票/ETF） | 能在页面添加任意股票/ETF 并看到数据状态 | ❌ | ❌ |
| **P1** 场外基金链路 | 同上，覆盖基金 | ✅ 蛋卷 | `fund_nav_daily` |
| **P2** 统一序列层 + 组合定义 | 三类资产能组成一个组合并存下来 | ❌ | `portfolio` 等 |
| **P3** 回测引擎 + 结果展示 | 点"运行"能出净值/指标；能对照两个 Run | ❌ | `backtest_run` 等 |
