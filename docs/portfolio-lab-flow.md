...
| **P2** 统一序列层 + 组合定义 | 三类资产能组成一个组合并存下来 | ❌ | `portfolio` 等 | ✅ **已完成**（`24a6326` / `9e59e66`；迁移 **0007**） |
| **P3** 回测引擎 + 结果展示 | 点"运行"能出净值/指标；能对照两个 Run | ❌ | `backtest_run` 等 | ✅ **已完成**（`709c635` + 本轮；迁移 **0008**） |

**P3 落地清单**（2026-09-19）：
- 引擎 `services/backtest.py`（纯算法）+ 编排 `services/backtest_service.py`（取数/校验/幂等落库）分层
- 端点 4 个：`POST /backtests`、`GET /backtests`、`GET /backtests/{run_id}`、`GET /backtests/compare`
- `portfolio_cache_refresh` job（**23:30**，排在 `fund_nav_sync` 之后 —— 三格要用当天最新净值）
- 前端 L2 八个展示区全部落地；「组合大爆炸」「组合实盘」按规格 **保持不做**
- 验收：`scripts/verify_engine_port.py` 对韭圈儿基准组合 **6/6 PASS**
- ⬜ **仍是二期**：`/backtests/cross-compare`（跨组合对照，须对齐交集区间）、「查看完整曲线」模式
